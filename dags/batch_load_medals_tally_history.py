...
with DAG(
    dag_id= "1.6-olympic_medels_tally_history_dag",
    ...
    def parse_files(file_paths):
        ...
        temp_path = Path("/usr/local/airflow/dags/temp5.pkl")
    ...
    def load_to_db(tuple_data_file_path):
        ...
        try:
            ...
            tuple_data = []
        valid_data = [record for record in tuple_data if len(record) == 9]
        if len(valid_data) != len(tuple_data):
            logger.warning(f"Filtered {len(tuple_data) - len(valid_data)} invalid records")
